...
STAGE_TITLES = {WorkflowStage.CONVERT: "Conversion",
...
def format_run_report(progress: RunProgress, run_dir: Path | None = None, *, include_header: bool = False) -> str:
    ...
    for stage in WorkflowStage:
        ...
        lines.append(f"{STAGE_TITLES[stage]}: {counts[StageStatus.COMPLETED]} completed / "
        ...
        for experiment, record in records:
            ...
            if explanation:
                ...
                details.append(f"{name}\n  {STAGE_TITLES[stage]}: "
                               f"{record.status.value} — {explanation}")
...
def _short_path(value: str, run_dir: Path | None) -> str:
    """
    Return a shortened path relative to the run directory, if possible.
    """
...
def _short_text(value: str, run_dir: Path | None) -> str:
    """
    Return a shortened text with paths relative to the run directory, if possible.
    """
