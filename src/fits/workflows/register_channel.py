...
def register_channel_one(settings: RegisterChannelSettings, exp_state: ExperimentState, step_profile: StepProfile, output_name: cst.FitsName) -> ExperimentState:
    if exp_state.image is None:
        failed_state = exp_state.with_error(cst.STEP_REGISTER_CHANNEL, f"ExperimentState for {exp_state.original_image} has no image set; cannot run {step_profile.step_name}.")
    ...
    try:
        ...
        new_st = exp_state.with_completed_step(cst.STEP_REGISTER_CHANNEL)
        ...
        return exp_state.with_error(cst.STEP_REGISTER_CHANNEL, str(e))


def run_register_channel(settings: RegisterChannelSettings, exp_state: list[ExperimentState], step_profile: StepProfile, output_name: cst.FitsName) -> list[ExperimentState]:
    ctx = get_ctx()
    ...
    def worker(st: ExperimentState) -> list[ExperimentState]:
        with use_ctx(ctx):
            return [register_channel_one(settings, st, step_profile, output_name)]
