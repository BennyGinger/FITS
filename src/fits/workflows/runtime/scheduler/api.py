...
def run_workflow_scheduler(effective_cfg: Mapping[str, Any],
    ...
    with (ThreadPoolExecutor(max_workers=cpu_workers) as cpu_executor,
        ...
        while cpu_ready or gpu_ready or cpu_running or gpu_running:
            submit_ready_tasks(ready=cpu_ready, running=cpu_running, max_running=cpu_workers,
                                executor=cpu_executor, runtime_steps=runtime_steps,
                                reporter=reporter)
            submit_ready_tasks(ready=gpu_ready, running=gpu_running, max_running=1,
                                executor=gpu_executor, runtime_steps=runtime_steps,
                                reporter=reporter)
            ...
            if not done:
                log_scheduler_heartbeat(cpu_ready=cpu_ready, gpu_ready=gpu_ready,
