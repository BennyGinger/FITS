...
def test_task_pool_maps_known_steps() -> None:
    assert _task_pool('convert') == 'cpu'
    assert _task_pool('segment') == 'gpu'
...
def test_run_workflow_scheduler_runs_single_enabled_step(monkeypatch) -> None:
    ...
    monkeypatch.setattr('fits.workflows.engines.scheduler.WORKFLOW_ORDER', ['convert'])
    ...
    monkeypatch.setattr(
        'fits.workflows.engines.scheduler.convert_one',
        lambda settings, exp_state, step_profile, output_name: [exp_state.with_completed_step(step_profile.step_name)],
    )
